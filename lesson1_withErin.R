...
print(surface_area_km2)
 
c(1:100)
seq(1,15)

my_vector = seq(from = 2, to = 10, by = 2)
print(my_vector)
my_vector_add7 = my_vector+7
my_vector_add7
my_vector2 = seq(from = 2, to = 6, by = 2)
my_vector2
Add_my_vectors = my_vector+my_vector2
Add_my_vectors
...
typeof(my_matrix2)
dim(my_matrix2)
my_matrix2[,2] #gives all rows corresponding to that col


#Array
my_array = array(c(1:18), dim=c(3,3,2))# 3rows, 3cols 2 sheets, creating 2 3x3 multidimentional arrays consisting of elements 1 to 18
my_array
class(my_array)  # the class or data structure is an "array"
typeof(my_array) # the array is filled with data of the type "integer"
my_array[2,3,2] # Best practice (3rd row, 2nd column of the 1st matrix)
...
#lists
list1 = list(c(31:45), "R", list(TRUE, c(FALSE, TRUE)))
list1
class(list1)
...
world_oceans
world_oceans[2,2] # Area of the Pacific
world_oceans$area_km2 # All depths
world_oceans$area_km2[2]
...
world_oceans$borders_usa

#snake_case uses lower case separated by underscore
#CamelCase uses uppercase to separate words
...
world_oceans$volume_km3
Traveled_oceans = data.frame(oceans = c("Atlantic", "Pacific", "Indian", "Arctic", "Southern"),
                              traveled = c(TRUE, FALSE, FALSE, FALSE, FALSE))#creating a data frame of personally traveled oceans
Traveled_oceans
Traveled_oceans$wish_to_travel = c(TRUE, TRUE, TRUE, TRUE, TRUE)#adding one more column
oceans_traveled = data.frame(oceans = c("Atlantic"))#or I could do it this way
oceans_traveled
world_oceans[c(1,2),]

#functions
round(9.756)
round(9.756, digits=2) 
round(9.756, 2)
